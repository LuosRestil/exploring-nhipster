...
describe('Author Controller', () => {
    ...
    it('/GET all authors ', async () => {
        const getEntities: AuthorDTO[] = (await request(app.getHttpServer())
            .get('/api/authors')
            .expect(200)).body;
    ...
    it('/GET authors by id', async () => {
        const getEntity: AuthorDTO = (await request(app.getHttpServer())
    ...
    it('/POST create authors', async () => {
        const createdEntity: AuthorDTO = (await request(app.getHttpServer())
            .post('/api/authors')
            .send(entityMock)
            .expect(201)).body;
    ...
    it('/PUT update authors', async () => {
        const updatedEntity: AuthorDTO = (await request(app.getHttpServer())
            .put('/api/authors')
            .send(entityMock)
            .expect(201)).body;
    ...
    it('/DELETE authors', async () => {
        const deletedEntity: AuthorDTO = (await request(app.getHttpServer())
